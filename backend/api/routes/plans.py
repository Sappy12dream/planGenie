...
async def get_all_plans(
    ...
    try:
        # Build query
        query = supabase.table("plans").select("*").eq("user_id", user_id).order("created_at", desc=True)
        ...
        for plan in plans_result.data:
            ...
            # Get resources
            resources_result = supabase.table("resources").select("*").eq("plan_id", plan_id).execute()
            ...
            plan_response = PlanResponse(
                ...
                resources=[ResourceResponse(**resource) for resource in resources_result.data],
...
async def update_plan_status(
    ...
    try:
        ...
        # Update status
        updated_plan = supabase.table("plans").update({"status": status}).eq("id", plan_id).execute()
